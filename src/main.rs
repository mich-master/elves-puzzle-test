...
fn digit_pattern_to_digit(p: u32) -> u32 {
    if p < 10 { p } else { p - 9 }
}

fn elves_line_value_2(line: &str) -> u32 {
    let ac = AhoCorasick::new(DIGITS).unwrap();

    let mut iter = ac.find_overlapping_iter(line);
    if let Some(m) = iter.next() {
        let first_digit: u32 = digit_pattern_to_digit(m.pattern().as_u32());
        let mut second_digit: u32 = first_digit;

        for m in iter {
            second_digit = digit_pattern_to_digit(m.pattern().as_u32());
        }
...
fn elves_puzzle_2(text: &str) -> u32 {
    text.lines()
        .fold(0, |sum, line| elves_line_value_2(line) + sum)
}

fn main() {
    let text = "jjfvnnlfivejj1
6fourfour
ninevbmltwo69
...
fn test_elves_puzzle_2() {
    ...
    assert_eq!(elves_puzzle_2(text), 281);

    let text = "6mnhhjtfbcpeightwozj";
    assert_eq!(elves_puzzle_2(text), 62);
}
